...
(function(win) {
	...
	var gb = function(option) {
		...
		self.Opt = {
			...
			playAColor: "red",
			playBColor: "blue",
			playAImg: "../img/game/playA.png",
			playBImg: "../img/game/playB.png",
		...
		self.unbindEvent = function(){
			self.contextObj.removeEventListener("touchstart", touchstartFunc);
			self.contextObj.removeEventListener("click", clickFunc);
			self.contextObj.removeEventListener("mousemove", mousemoveFunc);
			my.eventBinded = false;
		}
		// 允许下棋
		self.rebindEvent = function(){
			self.contextObj.addEventListener("touchstart", touchstartFunc);
			self.contextObj.addEventListener("click", clickFunc);
			self.contextObj.addEventListener("mousemove", mousemoveFunc);
			my.eventBinded = true;
		}
		 ...
		 * 游戏逻辑
		 */
		function logic(loc, iscomputer,isSend) {
			my.ChessDownNum++;
			//计算过的参数
			var c2d = new Object();
			if (!iscomputer) {
